...
import hk.enums.ActionId;
import hk.enums.ItemType;
import hk.enums.PageId;
import lombok.Data;
...
public class AppAction {
     ...
     /**
      * 根据概率随机返回布尔值
      * @param trueRate 为true的概率(0-100)
      * @return 随机布尔
      */

     private static Boolean getRandomBool(int trueRate) {
    	    RandomOptionGroup<Boolean> group = RandomOptionGroup.<Boolean>builder()
    	            .add(true, trueRate)
    	            .add(false, 100 - trueRate)
    	            .build();
    	    return group.getRandBoolValue();
    	}
     ...
     public  static  List<AppAction> buildList(AppPage appPage,Long startTs,Integer duringTime){
          ...
          Boolean ifFavor = getRandomBool(AppConfig.if_favor_rate);
          Boolean ifCart = getRandomBool(AppConfig.if_cart_rate);
          Boolean ifCartAddNum = getRandomBool(AppConfig.if_cart_add_num_rate);
          Boolean ifCartMinusNum = getRandomBool(AppConfig.if_cart_minus_num_rate);
          Boolean ifCartRm = getRandomBool(AppConfig.if_cart_rm_rate);
